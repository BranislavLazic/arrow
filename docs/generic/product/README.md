...
// HCons(head=1000, tail=HCons(head=900, tail=arrow.generic.HNil@fa4d642))
```

```kotlin
Account(1000, 900).toHListLabeled()
// HCons(head=Tuple2(a=balance, b=1000), tail=HCons(head=Tuple2(a=available, b=900), tail=arrow.generic.HNil@fa4d642))
```
...
// DeferredK(deferred=LazyDeferredCoroutine{New}@131c051c)
```
